...
- **THEN** 其定義位於獨立檔 `settings/drawer/DetailDrawer.tsx`，由 shell `import` 後以 `<DetailDrawer ... />` 渲染；與 shell 共用的 `DrawerState` 型別移至 `settings.types.ts` 供雙方 import；`DetailDrawer` 自共用模組 import 其資料/型別/icons/`StatusBadge`；shell（`Settings.tsx`）收斂為僅含導覽、路由、各面板接線與 drawer state 的殼層
