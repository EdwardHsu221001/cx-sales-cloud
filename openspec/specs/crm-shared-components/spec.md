...
#### Scenario: 含共用子元件的面板抽為獨立元件

- **WHEN** 檢視「使用者清單與角色指派」面板 `UsersPanel`（原巢狀、以 `<UsersPanel />` 渲染）
- **THEN** 其定義位於獨立檔 `settings/users/UsersPanel.tsx`，由 shell `import` 後以 `<UsersPanel showToast={...} onNavigate={...} onOpenUser={...} />` 渲染；其專屬 state（`selectedRows` 及 `toggleRow`/`toggleAll`、`userSearch`、衍生 `filteredUsers`/`activeCount`/`pendingCount`）下放為 local state，專屬 helper `PermChips` 隨面板搬入；開共用 `DetailDrawer` 的 `openUser` 以 `onOpenUser` prop 傳入；與 `DetailDrawer` 共用的 `StatusBadge` 移至 `settings.components.tsx`、其依賴 `STATUS_MAP` 移至 `settings.data.ts`，由 shell 與面板共同 import

#### Scenario: 含 route 依賴的面板抽為獨立元件

- **WHEN** 檢視含 route 依賴的面板 `ObjectsPanel`（物件管理員）與 `FieldsPanel`（單一物件欄位,route `/settings/objects/:api`）
- **THEN** 各自位於獨立檔 `settings/objects/ObjectsPanel.tsx`、`settings/fields/FieldsPanel.tsx`，由 shell `import` 後以 `<Panel ... />` 渲染；其專屬 state（`ObjectsPanel` 的 `objTab`、`FieldsPanel` 的 `fieldSearch`）下放為 local state；route 解析仍集中於 shell,`FieldsPanel` 以 prop `selectedObjApi` 收 shell 推導值、`ObjectsPanel` 以 prop `onOpenObject(api)` 收導航回呼（面板不 import `next/navigation`）；共用資料 `OBJ_ITEMS`/`OBJ_ICON_STYLE` 移至 `settings.data.ts`、型別 `ObjItem`/`FieldRow` 移至 `settings.types.ts`,`FieldsPanel` 專屬 `FIELDS_DATA` 隨面板搬入

#### Scenario: 模組層級但過大的元件抽為獨立檔

- **WHEN** 檢視已是模組層級但體積過大的多型詳情抽屜 `DetailDrawer`（依 `state.type` 顯示 user/profile/permset/flow/batch 五種內容,以 inline-call dispatch）
- **THEN** 其定義位於獨立檔 `settings/drawer/DetailDrawer.tsx`，由 shell `import` 後以 `<DetailDrawer ... />` 渲染；與 shell 共用的 `DrawerState` 型別移至 `settings.types.ts` 供雙方 import；`DetailDrawer` 自共用模組 import 其資料/型別/icons/`StatusBadge`（含其專屬的物件存取矩陣資料 `OBJECTS`/`ACCESS`）；shell（`Settings.tsx`）收斂為僅含導覽、路由、各面板接線與 drawer state 的殼層
