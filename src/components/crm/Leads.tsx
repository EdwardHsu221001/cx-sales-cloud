...
const LEADS: Lead[] = [
  {
    id: 1,
    name: '陳雅婷', initial: '陳', company: '緯創資通',
    contacts: ['email', 'phone'],
    score: 'hot', status: 'followed', source: '展覽活動',
    assigneeBg: 'linear-gradient(135deg,#60a5fa,#2563eb)',
    assigneeInitial: '張', assigneeName: '張志豪',
    canConvert: true, convertTitle: '採購經理',
  },
  {
    id: 2,
    name: '林志明', initial: '林', company: '研華科技',
    contacts: ['email', 'phone'],
    score: 'hot', status: 'toconvert', source: '業務介紹',
    assigneeBg: 'linear-gradient(135deg,#fbbf24,#f59e0b)',
    assigneeInitial: '林', assigneeName: '林俊傑',
    canConvert: true, convertTitle: '資訊部協理',
  },
  {
    id: 3,
    name: '黃柏翰', initial: '黃', company: '台達電子',
    contacts: ['email'],
    score: 'warm', status: 'contacted', source: '官網表單',
    assigneeBg: 'linear-gradient(135deg,#a78bfa,#7c3aed)',
    assigneeInitial: '陳', assigneeName: '陳美華',
    canConvert: false,
  },
  {
    id: 4,
    name: '王俊凱', initial: '王', company: '聯發科技',
    contacts: ['email', 'phone'],
    score: 'warm', status: 'overdue', source: '廣告投放',
    assigneeBg: 'linear-gradient(135deg,#60a5fa,#2563eb)',
    assigneeInitial: '張', assigneeName: '張志豪',
    canConvert: false,
  },
  {
    id: 5,
    name: '吳雅文', initial: '吳', company: '英業達',
    contacts: ['email'],
    score: 'cold', status: 'new', source: '官網表單',
    assigneeBg: 'linear-gradient(135deg,#a78bfa,#7c3aed)',
    assigneeInitial: '陳', assigneeName: '陳美華',
    canConvert: false,
  },
  {
    id: 6,
    name: '蔡宗翰', initial: '蔡', company: '仁寶電腦',
    contacts: ['email', 'phone'],
    score: 'cold', status: 'new', source: '展覽活動',
    assigneeBg: 'linear-gradient(135deg,#fbbf24,#f59e0b)',
    assigneeInitial: '林', assigneeName: '林俊傑',
    canConvert: false,
  },
...
function IconCardScan() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <circle cx="8" cy="11" r="2" /><path d="M14 10h4M14 14h4" />
    </svg>
  )
}
...
function IconBuilding() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <path d="M15 9h3a1 1 0 0 1 1 1v11" /><path d="M3 21h18M8 8h2M8 12h2M8 16h2" />
    </svg>
  )
}
function IconPerson() {
  ...
  return (
    <>
      ...
      <div className="cx-lead-head">
        <div>
          <h1>潛在客戶</h1>
          <div className="sub">管理尚未轉換的名單 — 依評分判斷優先順序，跟進後一鍵轉換為客戶、聯絡人與商機。</div>
        </div>
        <div className="actions">
          ...
          <div className="cx-quick-add-wrap" ref={addMenuRef}>
            ...
            <div className={`cx-quick-menu${addMenuOpen ? ' open' : ''}`}>
              {[
                { label: '手動新增', Icon: IconPersonAdd, bg: 'var(--cx-accent-soft)', color: 'var(--cx-accent)' },
                { label: '名片掃描', Icon: IconCardScan,  bg: 'var(--cx-success-soft)', color: '#0f9b6c' },
              ].map(({ label, Icon, bg, color }) => (
                <div key={label} className="cx-qm-item" onClick={() => { setAddMenuOpen(false); showToast(`已開啟「${label}」`) }}>
                  <span className="cx-qm-icon" style={{ background: bg, color }}><Icon /></span>
                  {label}
                </div>
              ))}
              <hr />
              <div className="cx-qm-item" onClick={() => { setAddMenuOpen(false); showToast('已開啟「CSV 匯入」') }}>
                <span className="cx-qm-icon" style={{ background: '#FFF4EA', color: '#c2691e' }}><IconFileCsv /></span>
                CSV 匯入
              </div>
            </div>
          </div>
      ...
      <div className="cx-stat-bar">
        <div className="cx-stat">
          <div className="cx-sic blue"><IconTarget /></div>
          <div><div className="cx-snum">32</div><div className="cx-slbl">全部潛客</div></div>
        </div>
        <div className="cx-stat">
          <div className="cx-sic red">🔥</div>
          <div><div className="cx-snum red">8</div><div className="cx-slbl">Hot 高潛力</div></div>
        </div>
        <div className="cx-stat">
          <div className="cx-sic orange"><IconClockCircle /></div>
          <div><div className="cx-snum orange">5</div><div className="cx-slbl">待跟進 &gt; 7 天</div></div>
        </div>
        <div className="cx-stat">
          <div className="cx-sic green"><IconCheckFat /></div>
          <div><div className="cx-snum green">6</div><div className="cx-slbl">本月已轉換</div></div>
        </div>
      </div>
      ...
      <div className="cx-filter-row">
        ...
        <div className="cx-filter-count">共 <b>32</b> 筆</div>
      </div>
      ...
      <div className="cx-lead-card">
          ...
          <tbody>
            {LEADS.map(lead => (
              <tr key={lead.id} className={selectedRows.has(lead.id) ? 'sel' : ''}>
                ...
                <td>
                  <div className="cx-contact-ic">
                    {lead.contacts.includes('email') && (
                      <span title="Email"><IconEmail /></span>
                    )}
                    {lead.contacts.includes('phone') && (
                      <span title="電話"><IconPhone /></span>
                    )}
                  </div>
                </td>
                <td>
                ...
                <td><span className="cx-lead-src">{lead.source}</span></td>
                <td>
                  <div className="cx-lead-assignee">
                    <div className="av" style={{ background: lead.assigneeBg }}>{lead.assigneeInitial}</div>
                ...
                <td>
                  <div className="cx-lead-ops">
                    {lead.canConvert ? (
                      ...
                      <>
                        <button className="cx-op-ic" title="編輯" onClick={() => showToast('「編輯」· 功能設計中')}>
                          <IconEdit />
                        </button>
                        <button className="cx-op-ic" title="更多" onClick={() => showToast('「更多」· 功能設計中')}>
                          <IconDots />
                        </button>
                      </>
                    )}
        ...
        <div className="cx-pager">
          <div className="info">顯示第 <b>1–6</b> 筆，共 <b>32</b> 筆</div>
          <div className="cx-pages">
            <button className="cx-pg nav" disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)}>
              <IconChevLeft />
            </button>
            {[1, 2, 3].map(p => (
              <button
            ...
            <button className="cx-pg nav" disabled={currentPage === 6} onClick={() => setCurrentPage(p => p + 1)}>
              <IconChevRight />
            </button>
          </div>
        </div>
        ...
        <div className="cx-modal">
          <div className="cx-modal-head">
            <div>
              ...
              <div className="ms">系統將以此潛客建立三筆關聯紀錄，並把原潛客標記為「已轉換」。轉換後可在帳號詳情頁追蹤後續。</div>
            </div>
          ...
          <div className="cx-modal-body">
            <div className="cx-conv-flow">
              ...
              <div className="cx-conv-card">
                <div className="cic" style={{ background: 'var(--cx-accent-soft)', color: 'var(--cx-accent)' }}>
                  <IconBuilding />
                </div>
                <div>
              ...
              <div className="cx-conv-arrow"><IconArrowDown /></div>
              {/* Contact */}
              <div className="cx-conv-card">
              ...
              {/* Opportunity */}
              <div className="cx-conv-card">
                <div className="cic" style={{ background: 'var(--cx-success-soft)', color: '#059669' }}>
                  <IconTrendUp />
                </div>
                <div>
          ...
          <div className="cx-modal-foot">
            ...
            <div className="grp">
              <button className="cx-btn-ghost" onClick={() => setModal(null)}>取消</button>
