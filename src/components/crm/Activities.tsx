...
interface FeedItem {
  ...
  title: string;
  desc: string;
  co: keyof typeof CO | null;
  person: string | null;
  who: keyof typeof ASSIGNEES;
  time: string;
  isTask?: boolean;
  done?: boolean;
}
...
const INIT_FEED: FeedGroup[] = [
  {
    ...
    items: [
      {
        type: 'task',
        title: '準備台積電智慧製造平台報價簡報',
        desc: '整理 POC 成效數據與導入時程，下午 4 點前完成初稿。',
        co: 'tsmc',
        person: '王俊傑',
        who: 'me',
        time: '09:10',
      ...
      },
      {
        type: 'call',
        title: '與王協理電話會議',
        desc: '確認智慧製造平台導入時程，對方希望 Q3 完成 POC，已同步給 PM。',
        co: 'tsmc',
        person: '王俊傑',
        who: 'zhang',
        time: '08:40',
      ...
      {
        type: 'mail',
        title: '寄送能源儀表板 Demo 連結',
        desc: '台達電子黃處長對即時能耗視覺化反應正面，下週安排內部簡報。',
        co: 'delta',
        person: '黃柏翰',
        who: 'chen',
        time: '08:05',
  ...
  },
  {
    ...
    items: [
      {
        type: 'meet',
        title: '聯發科雲端研發環境 — 合約協商會議',
        desc: '對方要求 SLA 提升至 99.9%，已回報主管評估可行性。',
        co: 'mtk',
        person: '林佳蓉',
        who: 'chen',
        time: '16:30',
      ...
      },
      {
        type: 'mail',
        title: '寄送資安監控系統提案報價單',
        desc: '報價 NT$ 1.2M，已附導入時程表與服務範圍說明。',
        co: 'tsmc',
        person: '李欣怡',
        who: 'zhang',
        time: '11:20',
      },
      {
        type: 'call',
        title: '緯創資通初次需求電訪',
        desc: '了解採購規模與導入時程，對方仍在比較階段，需積極跟進。',
        co: 'wis',
        person: '陳雅婷',
        who: 'zhang',
        time: '10:05',
  ...
  },
  {
    ...
    items: [
      {
        type: 'meet',
        title: '華碩全球客服系統 — 成交慶功與後續規劃',
        desc: '系統正式上線，回應時間縮短 35%，討論 AI 客服第二期合作意向。',
        co: 'asus',
        person: '劉雅琪',
        who: 'zhang',
        time: '週一 15:00',
      },
      {
        type: 'note',
        title: '研華科技標記為風險帳號',
        desc: '近 90 天無互動，原合約已到期未續，需主管介入挽回。',
        co: 'delta',
        person: '林志明',
        who: 'lin',
        time: '週一 09:30',
      ...
      {
        type: 'mail',
        title: '寄送聯發科 POC 結果報告',
        desc: '資料分析平台試行成效良好，查詢效能提升 40%。',
        co: 'mtk',
        person: '黃信宏',
        who: 'chen',
        time: '週一 10:10',
...
export default function Activities({ showToast }: { showToast: (msg: string) => void }) {
    ...
    setFeed((prev) =>
      prev.map((g, gi) =>
        gi !== groupIdx
          ...
          : {
              ...g,
              items: g.items.map((it, ii) => (ii !== itemIdx ? it : { ...it, done: !it.done })),
            }
      )
    );
  ...
  const visibleFeed: FeedGroup[] =
    filter === 'all'
      ? feed
      : feed
          .map((g) => ({
            ...g,
            items: g.items.filter((it) => it.type === filter),
          }))
    ...
    <>
      ...
      <div className="cx-lead-head" style={{ marginBottom: 18 }}>
        <div>
          <h1>活動與任務</h1>
          <div className="sub">
            所有通話、Email、會議與任務的統一時間軸 — 快速記錄每次互動，待辦一目了然。
          </div>
        </div>
      ...
      <div className="cx-stat-bar" style={{ marginBottom: 18 }}>
        <div className="cx-stat">
          ...
          <div>
            <div className="cx-snum">7</div>
            <div className="cx-slbl">今日待辦</div>
          </div>
        </div>
        <div className="cx-stat">
          <div className="cx-sic red">
            <IconClock />
          </div>
          <div>
            <div className="cx-snum red">{overdue}</div>
            <div className="cx-slbl">逾期任務</div>
          </div>
        </div>
        <div className="cx-stat">
          ...
          <div>
            <div className="cx-snum green">24</div>
            <div className="cx-slbl">本週已完成</div>
          </div>
        </div>
        <div className="cx-stat">
          ...
          <div>
            ...
            <div className="cx-slbl">未來 7 天會議</div>
          </div>
        </div>
      </div>
      ...
      <div className="cx-act-layout">
        ...
        <div>
          ...
          <div className="cx-feed-tabs">
            <div
              className={`cx-ftab${filter === 'all' ? ' on' : ''}`}
                            ...
                            <div className="cx-fmeta">
                              {item.co && (
                                <span className="cx-rel-chip">
                                  <span className="rl" style={{ background: CO[item.co].g }}>
                                    {CO[item.co].logo}
                                  </span>
                                  {CO[item.co].nm}
                                </span>
                              ...
                              <span className="cx-fwho">
                                <span className="wav" style={{ background: who.g }}>
                                  {who.av}
                                </span>
                                {who.nm}
                              </span>
          ...
          <div className="cx-side-card">
            ...
            {tasks.map((t, i) => (
              <div key={i} className={`cx-task${t.done ? ' done' : ''}`}>
                <div className={`cx-tchk${t.done ? ' on' : ''}`} onClick={() => toggleTask(i)}>
                  <IconCheck />
                </div>
